...
void bitmap::construct(MultiPolygon& mult, int width, float xpres, float xmax, int height,
                       float ypres, float ymax) {
    nbBlack = 0;
 ...
 * @param height
 */
bitmap::bitmap(const bool* nmap, int width, int height) : width(width), height(height),
    nbBlack(0) {
...
std::ostream& operator<<(std::ostream& s, const bitmap& bmap) {
    ...
    s << bmap.nbBlack << " blacks out of " << bmap.width* bmap.height << " points" <<
      std::endl;
    return s;
}
