...
void quadTree::construct(double x1, double y1, double x2, double y2, Shape& shape,
    ...
    */
    ...
    if (depth < maxDepth) {
        ...
        q1 = new quadTree(min_corner.x(), min_corner.y(), xm, ym, shape, precision, depth + 1,
                          maxDepth); //bottom left
        q2 = new quadTree(min_corner.x(), ym, xm, max_corner.y(), shape, precision, depth + 1,
                          maxDepth); //bottom right
        q3 = new quadTree(xm, ym, max_corner.x(), max_corner.y(), shape, precision, depth + 1,
                          maxDepth); //top    right
        q4 = new quadTree(xm, min_corner.y(), max_corner.x(), ym, shape, precision, depth + 1,
                          maxDepth); //top    left
...
quadTree::quadTree(double x1, double y1, double x2, double y2, Shape& shape,
                   float precision, int depth, int maxDepth)
...
quadTree::quadTree(Shape& shape, float precision)
    ...
    double area = (envelop.max_corner().x() - envelop.min_corner().x()) *
                  (envelop.max_corner().y() * envelop.max_corner().y());
