...
int main() {
    try {
        ...
        ASSERT(abs(shapes[1].getNthPoly(0).outer()[0].x() - 212.18985) < EPSILON, "Bad X translation");
        ASSERT(abs(shapes[1].getNthPoly(0).outer()[0].y() - 948.67991) < EPSILON, "Bad Y translation");

		//Testing internal group translation
		ASSERT(abs(abs(shapes[2].getNthPoly(0).outer()[0].x() - shapes[2].getNthPoly(1).outer()[0].x())
			   	- 19.19287) < EPSILON, "Internal group translation");
		ASSERT(abs(abs(shapes[2].getNthPoly(0).outer()[0].y() - shapes[2].getNthPoly(1).outer()[0].y())
